...
class NameView(View):
    ...
    def post(self, request):
        ...
        return True
        # return HttpResponseRedirect(reverse('bowling:AddRoll', args=(game.id,)))
...
class AddRoll(View):
    ...
    def get(self, request, game_id):
        ...
        return render(request, 'bowling/AddRoll.html', context)
    ...
    def post(self, request, game_id):
        ...
        if form.is_valid() and len(frames_passed) < AddRoll.NORMAL_FRAMES:
        ...
        elif len(frames_passed) == AddRoll.NORMAL_FRAMES:
            attempt1 = form.cleaned_data['attempt1']
            attempt2 = form.cleaned_data['attempt2']
            attempt3 = form.cleaned_data['attempt3']
            totalScore = int(attempt1)+int(attempt2)+int(attempt3)
            ...
            return HttpResponseRedirect(reverse('bowling:AddRoll', args=(game_id,)))
        return HttpResponseRedirect(reverse('bowling:AddRoll', args=(game_id,)))
...
def total_score(request, game_id):
    ...
    results = []
    results = re_calculate(frame_results[:AddRoll.SPECIAL_FRAME], results, False)
    #in case of un handled post requests after the 10 frames are played
    if len(frame_results) > AddRoll.SPECIAL_FRAME:
        game.total_score = sum(results[:AddRoll.SPECIAL_FRAME])
        game.save()
